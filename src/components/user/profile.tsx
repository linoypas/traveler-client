...
function UserProfile() {
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/auth/${userInfo._id}`, {
                method: "PUT",
                body: formData,
                headers: {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                const userResponse = await fetch(`http://localhost:3000/auth/${userId}`, {
                    headers: {
                      Authorization: `Bearer ${localStorage.getItem('accessToken')}`
                    }
                  });
                ...
                const postsResponse = await fetch(`http://localhost:3000/posts?owner=${userId}`);
                if (!postsResponse.ok) {
        ...
        <div className="container">
            <div className="text-center my-12 flex flex-col items-center">
                <div className="relative">
                    {userInfo.image ? (
                        <img
                        src={userInfo.image ? `http://localhost:3000${userInfo.image}` : "default-avatar.png"}
                        alt="Profile"
