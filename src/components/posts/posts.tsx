...
const Posts = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const postsReponse = await fetch(`https://node43.cs.colman.ac.il:443/posts`, {
          method: 'GET',
          headers: {
          ...
          {posts.map((post) => (
            <Link to={`/posts/${post._id}`} key={post._id} className="post-card-link">
              <div className="post-card">
                {post.image ? (
                  <img src={`https://node43.cs.colman.ac.il:443${post.image}`} alt={post.title} className="post-image" />
                ) : (
